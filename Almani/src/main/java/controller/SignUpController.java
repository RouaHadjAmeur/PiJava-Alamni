...
public class SignUpController {
    ...
    public void initialize() {
        ...
        nomField.textProperty().addListener((obs, oldVal, newVal) -> {
            if (!TEXT_ONLY_PATTERN.matcher(newVal).matches()) {
    ...
    private void handleSignUp(ActionEvent event) {
        ...
        // Vérification que nom et prénom ne contiennent que des lettres, espaces, tirets ou apostrophes
        if (!TEXT_ONLY_PATTERN.matcher(nom).matches() || !TEXT_ONLY_PATTERN.matcher(prenom).matches()) {
            showAlert(Alert.AlertType.ERROR, "Champs invalides",
                    "Le nom et le prénom doivent contenir uniquement des lettres, espaces, tirets ou apostrophes.");
            return;
        }
